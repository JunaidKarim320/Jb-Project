...
namespace Spirit604.CityEditor.Road
{
    public partial class RoadSegmentCreator : MonoBehaviour
    {
        ...
        private List<Path> TryToCreateStraightPath(int currentNodeIndex, int laneIndex, int customNextIndex = -1)
        {
            ...
            List<Path> straightPaths = new List<Path>();
        ...
        private List<Path> TryToCreateRightPath(int currentNodeIndex, int laneCount, int laneIndex, int customNextIndex = -1)
        {
            ...
            if (!CanConnectTurnLaneIndex(currentNodeIndex, laneIndex, currentNode.LaneCount, false) && CreateTrafficNodeCount != 2)
                return null;
...
#pragma warning disable CS0162
                if (ProjectConstants.LaneHandDirection == 1)
                {
                    ...
                    {
                        nextLaneIndex = Mathf.Clamp(nextLaneIndex, 0, subLaneCount - 1);
                    }
...
#pragma warning restore CS0162
        ...
        public void ClearOffsetPaths()
        {
            for (int i = 0; i < createdTrafficNodes.Count; i++)
            {
                ...
                {
                    for (int laneIndex = 0; laneIndex < createdTrafficNodes[i].Lanes.Count; laneIndex++)
                    {
                        for (int n = 0; n < createdTrafficNodes[i].Lanes[laneIndex].paths?.Count; n++)
                        {
                            if (createdTrafficNodes[i].Lanes[laneIndex].paths != null && createdTrafficNodes[i].Lanes[laneIndex].paths.Count > n)
                            {
                                var path = createdTrafficNodes[i].Lanes[laneIndex].paths[n];
