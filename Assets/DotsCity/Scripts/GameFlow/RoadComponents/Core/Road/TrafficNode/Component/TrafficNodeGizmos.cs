        ...
        private void DrawArrows()
        {
            if (!isOneWay)
            {
                for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
                {
                    ...
                    DrawArrow(startPosition, this.GetNodeForward(1), CityEditorSettings.ArrowColor);
                }
            }

            int side = isOneWay && !isEndOfOneWay ? 1 : -1;

            for (int laneIndex = 0; laneIndex < laneCount; laneIndex++)
            {
                var startPosition = TrafficNodeExtension.GetLanePosition(this, laneIndex, true);
                DrawArrow(startPosition, this.GetNodeForward(side), IsOneWay && HasRightLanes ? CityEditorSettings.ArrowColor : CityEditorSettings.ExternalArrowColor);
            }
        }
