...
namespace Spirit604.Gameplay.Road
{
    ...
    public partial class TrafficNode : MonoBehaviour, IBakeRoad
    {
        ...
        public void Bake()
        {
            ...
            if (HasRightLanes)
            {
                ...
                if (trafficNodeType != TrafficNodeType.DestroyVehicle)
                {
                    ...
                    {
                        hasLanes = false;
                        UnityEngine.Debug.Log($"TrafficNode InstanceID {this.GetInstanceID()} has empty right lanes.{TrafficObjectFinderMessage.GetMessage()}");
                    }
                }
            ...
            if (HasLeftLanes)
            {
                ...
                if (trafficNodeType != TrafficNodeType.DestroyVehicle)
                {
                    ...
                    {
                        UnityEngine.Debug.Log($"TrafficNode InstanceID {this.GetInstanceID()} has empty left lanes. Make sure that the traffic node is connected to another segment.{TrafficObjectFinderMessage.GetMessage()}");
                    }
                }
#endif
            ...
            this.IterateAllLanes((data, laneIndex, external) =>
            {
                if (hasAnyRightPath && !external || hasAnyLeftPath && external)
                {
                    if (data == null || data.paths.Count == 0)
                    {
                        ...
                        UnityEngine.Debug.Log($"TrafficNode InstanceID {this.GetInstanceID()} {laneText} lane '{laneIndex}' null or empty.{TrafficObjectFinderMessage.GetMessage()}");
                    }
                }
        ...
        public void ConnectSegments(AutoConnectionSettings autoConnectionSettings)
        {
            ...
            if (connectedTrafficNode != null && autoConnectionSettings.ConnectCrosswalks)
            {
