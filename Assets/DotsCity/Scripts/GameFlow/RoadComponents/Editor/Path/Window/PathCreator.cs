...
namespace Spirit604.CityEditor.Road
{
    public class PathCreator : EditorWindowBase
    {
        ...
        private const float DefaultSideDotDirectionValue = 0.51f;
        ...
        [SerializeField] private bool showForbiddenPath = true;

        [SerializeField] private bool showOverridenPath = true;
        ...
        private void ShowVisualSettings(SerializedObject so)
        {
            System.Action visualSettingsCallback = () =>
            {
                ...
                EditorGUILayout.PropertyField(so.FindProperty(nameof(showForbiddenPath)));
                EditorGUILayout.PropertyField(so.FindProperty(nameof(showOverridenPath)));
        ...
        private void TryToCalculateSideConnection()
        {
            ...
            float faceDotDirection = Vector3.Dot(sourceTrafficNode.transform.forward, targetTrafficNode.transform.forward);

            if (sameCrossRoad)
            {
                sourceNodeSide = ConnectSideType.DefaultSide;

                if (!sourceTrafficNode.IsOneWay)
                {
                    if (faceDotDirection <= DefaultSideDotDirectionValue)
                    {
                        targetNodeSide = ConnectSideType.DefaultSide;
                    }
                    else
                    {
                        var connectDir = (targetTrafficNode.transform.position - sourceTrafficNode.transform.position).normalized;
                        var connectionDot = Vector3.Dot(sourceTrafficNode.transform.forward, connectDir);
                    ...
                    if (targetTrafficNode.IsOneWay)
                    {
                        if (faceDotDirection > 0)
                        {
                            sourceNodeSide = ConnectSideType.DefaultSide;
                        }
                        else
                ...
                {
                    if (sourceTrafficNode.IsEndOfOneWay)
                    {
                        sourceNodeSide = ConnectSideType.ExternalSide;
                    }
        ...
        public bool CreatePath()
        {
            ...
            switch (connectionType)
            {
                ...
                case ConnectionType.OneDirectionConnect:
                    {
                        lastPath = CreateDirecionPaths(sourceTrafficNode, targetTrafficNode, isExternal);
                ...
                case ConnectionType.TwoDirectionConnect:
                    {
                        lastPath = CreateDirecionPaths(sourceTrafficNode, targetTrafficNode, isExternal);
                        lastPath = CreateDirecionPaths(targetTrafficNode, sourceTrafficNode, TargetConnectionType == ConnectSideType.ExternalSide);
        ...
        private Path CreateDirecionPaths(TrafficNode sourceTrafficNode, TrafficNode targetTrafficNode, bool isExternal)
        {
            Path lastPath = null;
        ...
        private bool CanDrawConnectionPath(TrafficNode sourceTrafficNode, TrafficNode targetTrafficNode, bool sourceIsRightSide, int laneIndex, int connectionLaneIndex, out Color color)
        {
            ...
            var canDraw = !hasPath || showForbiddenPath || overrideType == OverrideType.Allowed;
