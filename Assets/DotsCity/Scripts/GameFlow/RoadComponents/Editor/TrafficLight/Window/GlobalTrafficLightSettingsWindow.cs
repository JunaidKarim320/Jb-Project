...
namespace Spirit604.CityEditor.Level
{
    public class GlobalTrafficLightSettingsWindow : EditorWindowBase
    {
        ...
        protected override void OnEnable()
        {
            ...
            {
                var roadNodeParent = roadNodeParents[0];
                trafficLightCrossRoads = roadNodeParent.gameObject.GetComponentsInChildren<TrafficLightCrossroad>();
        ...
        private void FindLightHandlers()
        {
            if (lightIndex != -1)
            {
                trafficLightHandlers = ObjectUtils.FindObjectsOfType<TrafficLightHandler>().Where(item => item.RelatedLightIndex == lightIndex).ToList();
            }
            else
            {
                trafficLightHandlers = ObjectUtils.FindObjectsOfType<TrafficLightHandler>().ToList();
            }
        }
        ...
        private void ReplaceSettings()
        {
            ...
            UnityEngine.Debug.Log($"GlobalTrafficLightSettings. {count} containers replaced.");
        }
