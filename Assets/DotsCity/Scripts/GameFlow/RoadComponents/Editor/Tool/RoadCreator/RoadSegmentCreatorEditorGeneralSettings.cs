...
namespace Spirit604.CityEditor.Road
{
    public partial class RoadSegmentCreatorEditor : Editor
    {
        private void ShowGeneralSettings(bool showType = true)
        {
            ...
            if (!customTrafficNodeSettings)
            {
                DrawDefaultRoadSettings(configSo);

                DrawCustomRoadSettings();

                ShowCrosswalkSettings(configSo);
            }
            else
            {
                DrawCustomRoadSettings();
        ...
        private void DrawDefaultRoadSettings(SerializedObject configSo)
        {
            InspectorExtension.DrawGroupBox("Road Settings", () =>
            {
                ...
                if (!creator.customLaneWidthEnabled)
                {
                    EditorGUILayout.PropertyField(configSo.FindProperty("laneWidth"));
                }
                else
                {
                    EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(creator.customLaneWidth)));
                }
                ...
                EditorGUI.PropertyField(r, serializedObject.FindProperty(nameof(creator.customLaneWidthEnabled)), GUIContent.none);
        ...
        private void DrawCustomRoadSettings()
        {
            InspectorExtension.DrawGroupBox("Custom Settings", () =>
            {
                ...
                if (creator.SubLaneSupport())
                {
                    ...
                    var customSubSaneWidthProp = configSo.FindProperty("customSubLaneWidth");
                    ...
                    if (customSubSaneWidthProp.boolValue)
                    {
                        ...
                        EditorGUILayout.PropertyField(configSo.FindProperty("subLaneWidth"));
                        EditorGUI.indentLevel--;
                    }
        ...
        private void ShowCrosswalkSettings(SerializedObject configSo)
        {
            InspectorExtension.DrawGroupBox("Pedestrian Node Settings", () =>
            {
                ...
                if (creator.addPedestrianNodes)
                {
                    ...
                    EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(creator.uniqueCrossWalkOffset)));

                    if (!creator.uniqueCrossWalkOffset)
                    {
                        EditorGUILayout.PropertyField(configSo.FindProperty("crossWalkOffset"));
                    }
                    else
                    {
                        EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(creator.customCrossWalkOffset)));
                    }
