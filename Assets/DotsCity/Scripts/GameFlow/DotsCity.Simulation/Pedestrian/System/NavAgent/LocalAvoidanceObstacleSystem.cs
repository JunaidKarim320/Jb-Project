...
namespace Spirit604.DotsCity.Simulation.Pedestrian
{
...
#if REESE_PATH
    [UpdateAfter(typeof(UpdateNavAgentTargetSystem))]
#else
    ...
    public partial struct LocalAvoidanceObstacleSystem : ISystem
    {
...
#if REESE_PATH
        private SystemHandle updateNavAgentTargetSystem;
#endif
...
#if REESE_PATH
            updateNavAgentTargetSystem = state.WorldUnmanaged.GetExistingUnmanagedSystem<UpdateNavAgentTargetSystem>();
#endif
        ...
        void ISystem.OnUpdate(ref SystemState state)
        {
...
#if REESE_PATH
            ref var updateNavAgentTargetSystemRef = ref state.WorldUnmanaged.ResolveSystemStateRef(updateNavAgentTargetSystem);
            var depJob = JobHandle.CombineDependencies(updateNavAgentTargetSystemRef.Dependency, carHashMapSystemRef.Dependency, npcRecalculateNavTargetSystemRef.Dependency);
#else
            ...
            state.Dependency = avoidanceJob.Schedule(updateQuery, depJob);
        }
                    ...
                    if (CarHashMapSingleton.CarHashMap.TryGetFirstValue(pedestrianKey, out var carHashEntity, out var nativeMultiHashMapIterator))
                    {
                        do
                        {
                            ...
                            if (isIntersect)
                            {
                                ...
                                {
                                    float avoidanceOffset = ObstacleAvoidanceSettingsReference.SettingsReference.Value.ObstacleAvoidanceOffset * iter;
                                    ...
                                    if (iter == 1)
                                    {
                                        var dist = math.distancesq(point1, worldTransform.Position.Flat());

                                        if (dist < ObstacleAvoidanceSettingsReference.SettingsReference.Value.AchieveDistanceSQ)
                                        {
                                            continue;
                                        }
                                    }
        ...
        private static void GetOverlappedBoundsData(in NativeParallelMultiHashMap<int, CarHashEntityComponent> carHashMap, in CarHashEntityComponent sourceCarHashEntityComponent, in NativeList<int> keys, in float3 sourcePosition, in quaternion sourceRotation, in BlobAssetReference<ObstacleAvoidanceSettingsData> config, out bool overlapFound, out Bounds newBounds)
        {
            ...
            var targetOffset = config.Value.ObstacleAvoidanceOffset;
