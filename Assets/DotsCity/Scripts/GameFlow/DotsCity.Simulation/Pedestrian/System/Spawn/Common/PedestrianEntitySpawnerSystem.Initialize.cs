...
namespace Spirit604.DotsCity.Simulation.Pedestrian
{
    public partial class PedestrianEntitySpawnerSystem : SystemBase
    {
        ...
        private void InitCache()
        {
            ...
            if (EntityManager.CreateEntityQuery(typeof(DummyPrefabData)).TryGetSingletonEntity<Entity>(out var dummyPrefabEntity))
            {
                dummyRenderMesh = EntityManager.GetSharedComponentManaged<MyRenderMesh>(dummyPrefabEntity);
                World.GetExistingSystemManaged<DummySkinProviderSystem>().Initialize(dummyRenderMesh);
            }
            ...
            CapacityNodeHashMap = new NativeParallelHashMap<Entity, int>(roadStatConfig.PedestrianNodeTotal, Allocator.Persistent);
