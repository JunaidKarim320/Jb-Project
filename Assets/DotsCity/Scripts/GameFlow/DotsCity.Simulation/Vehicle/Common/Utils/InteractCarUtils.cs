...
namespace Spirit604.DotsCity.Simulation.Car
{
    public static class InteractCarUtils
    {
        ...
        public static void EnterCar(ref EntityCommandBuffer commandBuffer, ref EntityManager entityManager, ref BlobAssetReference<CarSharedConfig> soundConfig, Entity soundPrefabEntity, Entity entity, bool inViewOfCamera, bool ignite, int runtimeIndex, float3 position)
        {
            ...
            EnterCar(ref commandBuffer, ref soundConfig, soundPrefabEntity, entity, inViewOfCamera, ignite, runtimeIndex, position);
        }
            ...
            float3 position)
        {
            ...
            if (inViewOfCamera)
            {
                ...
                if (soundId != -1)
                {
                    var soundEntity = commandBuffer.CreateSoundEntity(soundPrefabEntity, soundId, position);
                }
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ExitCar(ref EntityCommandBuffer commandBuffer, ref BlobAssetReference<CarSharedConfig> soundConfig, Entity soundPrefabEntity, float3 position, int runtimeIndex)
        {
