                ...
                in CarModelComponent carModelComponent)
            {
                ...
                switch (carIgnitionData.IgnitionState)
                {
                    ...
                    case IgnitionState.IdleBeforeIgnite:
                        {
                            if (CurrentTime >= carIgnitionData.EndTime)
                            {
                                ...
                                if (SoundLevelConfigReference.Config.Value.HasSounds)
                                {
                    ...
                    case IgnitionState.EngineStarted:
                        {
                            if (SoundLevelConfigReference.Config.Value.HasSounds && startedTimeDuration > 0 && carSoundData.CarSoundType == CarSoundType.Driving && SoundVolumeLookup.HasComponent(carSoundData.SoundEntity))
                            {
