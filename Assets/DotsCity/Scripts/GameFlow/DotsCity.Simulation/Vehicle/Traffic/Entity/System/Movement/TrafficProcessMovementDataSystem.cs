        ...
        public struct TrafficMovementJob : IJobChunk
        {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                for (var index = 0; index < chunk.Count; index++)
                {
                    ...
                    float signedAngle = Vector3.SignedAngle(targetDirection.Flat(), desiredMovementForward.Flat(), Vector3.up) * desiredMovementDirectionSign;

                    float maxSteerAngle = trafficSettings.MaxSteerAngle;

                    signedAngle = math.radians(signedAngle);
                    signedAngle = math.clamp(-signedAngle, -maxSteerAngle, maxSteerAngle);

                    movementComponent.DesiredSteeringAngle = signedAngle;
                        ...
                        if (hasRotationLerp)
                        {
                            switch (TrafficCommonSettingsReference.Reference.Value.SimplePhysicsType)
                            {
                                case SimplePhysicsSimulationType.CarInput:
                                    float steeringFactor = math.abs(movementComponent.SteeringAngle) / maxSteerAngle;
