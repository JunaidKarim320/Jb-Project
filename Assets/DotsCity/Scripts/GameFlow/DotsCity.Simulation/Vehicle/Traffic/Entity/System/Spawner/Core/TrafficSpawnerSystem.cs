...
namespace Spirit604.DotsCity.Simulation.Traffic
{
    ...
    public partial class TrafficSpawnerSystem : EndInitSystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            carHashMapSystem = World.DefaultGameObjectInjectionWorld.Unmanaged.GetExistingUnmanagedSystem<CarHashMapSystem>();
        ...
        private JobHandle CreateSpawnJob(int spawnCount, bool initialSpawn, TrafficSpawnParams customTrafficSpawnParams, bool userCreated = false)
        {
            ...
            ref var carHashMapSystemRef = ref World.DefaultGameObjectInjectionWorld.Unmanaged.ResolveSystemStateRef(carHashMapSystem);
        ...
        private int GetSpawnCount(bool initialSpawn)
        {
            ...
            if (!initialSpawn)
            {
                maxCount = math.min(maxCount, spawnerConfig.Reference.Value.MaxSpawnCountByIteration);
            }

            if (spawnerConfig.Reference.Value.MaxCarsPerNode > 0)
            {
                var nodeQuery = GetNodeQuery(initialSpawn);
                var currentMax = spawnerConfig.Reference.Value.MaxCarsPerNode * nodeQuery.CalculateEntityCount();

                maxCount = math.min(maxCount, currentMax);
            }

            return math.clamp(maxCount - currentTrafficsCount, 0, int.MaxValue);
        }
