...
namespace Spirit604.DotsCity.Simulation.Traffic.Obstacle
{
    public static class TrafficObstacleUtils
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ObstacleResult CheckForNeighborPath(
            in NativeParallelMultiHashMap<int, CarHashData> carHashMap,
            in PathGraphSystem.Singleton graph,
            in BlobAssetReference<TrafficObstacleConfig> configRef,
            in CarHashData currentCarHash)
        {
            ...
            if (currentPath.NeighborCount == 0)
                return default;

            var neighborPathInfoList = graph.GetNeighborPaths(in currentPath);
            ...
            float3 dstPoint2 = default;
            VectorExtensions.Line routeLine = default;
            VectorExtensions.Line routeLine2 = default;

            for (int i = 0; i < neighborPathInfoList.Length; i++)
            {
                if (carHashMap.TryGetFirstValue(neighborPathInfoList[i], out var targetCarHash, out var nativeMultiHashMapIterator))
                {
                    do
                    {
                        ...
                        if (targetCloseToCurrentCar && targetCloserToEndOfPath)
                        {
                            ...
                            if (inFrontOfView)
                            {
                                if (dstPoint.Equals(float3.zero))
                                {
                                    ...
                                    dstPoint2 = dstPoint + right * currentCarHash.Bounds.Size.x * sizeMult;
                                    dstPoint = dstPoint - right * currentCarHash.Bounds.Size.x * sizeMult;
                                    ...
                                    routeLine = new VectorExtensions.Line(sourcePoint1, dstPoint.Flat());
                                    routeLine2 = new VectorExtensions.Line(sourcePoint2, dstPoint2.Flat());
                                }
                                ...
                                intersectPoint = VectorExtensions.LineWithSquareIntersect(routeLine2, obstacleSquare.Square, true);
            ...
            bool avoidCrossroadJam)
        {
            ...
            for (int i = 0; i < intersectPathInfoList.Length; i++)
            {
                ...
                Bounds currentBoundsNearIntersectPoint = default;
                    ...
                    case IntersectCalculationMethod.Bounds:
                        {
                            ...
                            if (currentCarHash.HasState(State.AtTrafficArea))
                            {
                            ...
                            currentBoundsNearIntersectPoint = ObstacleLayoutHelper.GetRotatedBound(intersectPathInfo.IntersectPosition.Flat(), rotationAtPoint, currentCarHash.Bounds.Size);
                            break;
                        }
                }
                ...
                if (carHashMapLocal.TryGetFirstValue(intersectPathInfo.IntersectedPathIndex, out var targetCarHash, out var nativeMultiHashMapIterator))
                {
                    do
                    {
                        ...
                        bool sameTarget = (destinationComponent.CurrentNode != Entity.Null && destinationComponent.CurrentNode.Index == targetCarHash.CurrentNodeIndex) || destinationComponent.DestinationNode.Index == targetCarHash.CurrentNodeIndex || destinationComponent.DestinationNode.Index == targetCarHash.DestinationNodeIndex;
                            ...
                            case IntersectCalculationMethod.Bounds:
                                {
                                    ...
                                    var targetBoundsCurrent = ObstacleLayoutHelper.GetRotatedBound(targetCarHash.Position.Flat(), targetCarHash.Rotation, targetCarHash.Bounds.Size, targetBoundsOffset);

                                    targetTooCloseToIntersectPoint = currentBoundsNearIntersectPoint.Intersects(targetBoundsCurrent);
                                    ...
                                    if (!targetCarIsIdle)
                                    {
                                        if (!currentBoundsFound)
                                        {
                                            ...
                                            currentBounds = ObstacleLayoutHelper.GetRotatedBound(currentCarHash.Position.Flat(), currentCarHash.Rotation, currentCarHash.Bounds.Size, configRef.Value.SizeOffsetToIntersectPoint);
                                        }
                                    }
                                    else
                                    {
                                        currentBounds = ObstacleLayoutHelper.GetRotatedBound(currentCarHash.Position.Flat(), currentCarHash.Rotation, currentCarHash.Bounds.Size, 0);
                                    }

                                    var targetBoundsNearIntersectPoint = ObstacleLayoutHelper.GetRotatedBound(intersectPathInfo.IntersectPosition.Flat(), targetCarHash.Rotation, targetCarHash.Bounds.Size);
                                    ...
                                    if (sameTarget && !currentCarNearIntersectPoint && !targetTooCloseToIntersectPoint)
                                    {
                                        ...
                                        if (targetNotPassedIntersectPoint && atStopPosition && !isIdle)
                                        {
                                            ...
                                            var projectBounds = ObstacleLayoutHelper.GetRotatedBound(projectPoint, targetCarHash.Rotation, targetCarHash.Bounds.Size);
                                            ...
                                            var projectBounds2 = ObstacleLayoutHelper.GetRotatedBound(projectPoint2, currentCarHash.Rotation, currentCarHash.Bounds.Size);
