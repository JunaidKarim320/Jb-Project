...
{
    [WithNone(typeof(TrafficChangingLaneEventTag), typeof(TrafficIdleTag), typeof(TrafficCustomLocomotion))]
    ...
    public partial struct ObstacleCalcJob : IJobEntity
    {
            ...
            in TrafficPathComponent trafficPathComponent)
        {
            ...
            if (TrafficAreaAlignedLookup.HasComponent(currentEntity))
            {
            ...
            {
                obstacleResult = CheckForNeighborPath(
                    in CarHashMapParallel,
                    in Graph,
                    in configRef,
                    in currentCarHashData);
