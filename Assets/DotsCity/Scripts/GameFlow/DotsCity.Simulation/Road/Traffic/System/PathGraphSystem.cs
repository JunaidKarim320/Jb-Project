...
namespace Spirit604.DotsCity.Simulation.Road
{
    public partial struct PathGraphSystem : ISystem, ISystemStartStop
    {
        ...
        private NativeArray<int> allNeighborPaths;
        ...
        private UnsafeHashMap<int, NativeArrayIndexContainer> allNeighborPaths;
        ...
        public void OnStartRunning(ref SystemState state)
        {
            ...
            var allNeighborPathsTemp = new NativeList<int>(Allocator.TempJob);
            ...
            allNeighborPaths = new UnsafeHashMap<int, NativeArrayIndexContainer>(hashMapCapacity, Allocator.Persistent);
            ...
            for (int pathIndex = 0; pathIndex < paths.Length; pathIndex++)
            {
                ...
                var pathData = new PathData()
                {
                    ...
                    NeighborCount = path.NeighborPathsIndexes.Length,
                ...
                for (int j = 0; j < path.NeighborPathsIndexes.Length; j++)
                {
                    allNeighborPathsTemp.Add(path.NeighborPathsIndexes[j]);
                }
                ...
                neighborIndex += path.NeighborPathsIndexes.Length;
                ...
                if (path.NeighborPathsIndexes.Length > 0)
                {
                    var container = new NativeArrayIndexContainer()
                    {
                        Data = new NativeList<int>(path.NeighborPathsIndexes.Length, Allocator.Persistent)
                    };

                    for (int j = 0; j < path.NeighborPathsIndexes.Length; j++)
                    {
                        container.Data.Add(path.NeighborPathsIndexes[j]);
                    }

                    allNeighborPaths.Add(pathIndex, container);
                }
            ...
            allNeighborPaths = allNeighborPathsTemp.ToArray(Allocator.Persistent);
            ...
            var singleton = new PathGraphSystem.Singleton()
            {
                ...
                allNeighborPaths = this.allNeighborPaths,
        ...
        private void Dispose()
        {
            ...
            foreach (var item in allNeighborPaths)
                if (item.Value.Data.IsCreated) item.Value.Data.Dispose();
            ...
            if (allNeighborPaths.IsCreated) allNeighborPaths.Dispose();
