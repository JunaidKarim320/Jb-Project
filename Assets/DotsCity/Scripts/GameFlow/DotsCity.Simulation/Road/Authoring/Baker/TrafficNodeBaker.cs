...
namespace Spirit604.DotsCity.Simulation.Road.Authoring
{
    ...
    public class TrafficNodeBaker : Baker<TrafficNode>
    {
        ...
        public override void Bake(TrafficNode trafficNode)
        {
            ...
            {
                Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad?.name}', " +
                          $"Traffic node '{trafficNode.name}', InstanceID {trafficNode.GetInstanceID()} has invalid right lanes{TrafficObjectFinderMessage.GetMessage()}");
            }

            if (trafficNode.HasRightLanes && trafficNode.Lanes == null)
            {
                Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad?.name}', " +
                          $"Traffic node '{trafficNode.name}', InstanceID {trafficNode.GetInstanceID()} right lanes is null{TrafficObjectFinderMessage.GetMessage()}");
            }

            if (!trafficNode.HasLeftLanes && trafficNode.ExternalLanes?.Count > 0)
            {
                Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad?.name}', " +
                          $"Traffic node '{trafficNode.name}', InstanceID {trafficNode.GetInstanceID()} has invalid left lanes{TrafficObjectFinderMessage.GetMessage()}");
            }

            if (trafficNode.HasLeftLanes && trafficNode.ExternalLanes == null)
            {
                Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad?.name}', " +
                          $"Traffic node '{trafficNode.name}', InstanceID {trafficNode.GetInstanceID()} left lanes is null{TrafficObjectFinderMessage.GetMessage()}");
            }
        ...
        private Entity CreateTrafficNodeEntity(bool subNode = false)
        {
            ...
            AddComponent(entity, typeof(TrafficNodeAvailableComponent));
        ...
        private void InitializeRightNodePaths(TrafficNode sourceTrafficNode, int laneIndex)
        {
            if (sourceTrafficNode.IsOneWay && (sourceTrafficNode.Lanes?.Count == 0 || sourceTrafficNode.IsEndOfOneWay))
            {
                ...
                {
                    Debug.Log($"Crossroad: '{sourceTrafficNode.TrafficLightCrossroad.name}' TrafficNode: {sourceTrafficNode.name} InstanceID {sourceTrafficNode.GetInstanceID()} LaneIndex {laneIndex} OneWay External path is null{TrafficObjectFinderMessage.GetMessage()}");
                }
            }
            else
            {
                if (sourceTrafficNode.Lanes.Count == 0)
                {
                    ...
                    {
                        if (sourceTrafficNode.TrafficLightCrossroad)
                        {
                            Debug.Log($"Crossroad: '{sourceTrafficNode.TrafficLightCrossroad.name}' TrafficNode: {sourceTrafficNode.name} InstanceID {sourceTrafficNode.GetInstanceID()} doesn't have lanes{TrafficObjectFinderMessage.GetMessage()}");
                        }
                        else
                        {
                            Debug.Log($"{sourceTrafficNode.name} doesn't have lanes");
                        }
                    }
                        ...
                        case PathConnectionType.PathPoint:
                            {
                                ...
                                {
                                    Debug.Log($"Crossroad: '{sourceTrafficNode.TrafficLightCrossroad.name}' TrafficNode: {sourceTrafficNode.name} InstanceID {sourceTrafficNode.GetInstanceID()} Path {path.name} Connected path not found{TrafficObjectFinderMessage.GetMessage()}");
                                }
        ...
        private void InitializeLeftNodePaths(TrafficNode trafficNode, int laneIndex)
        {
            ...
            {
                Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad.name}', External lanes: out of lane capacity {trafficNode.ExternalLanes.Count} TrafficNode InstanceID {trafficNode.GetInstanceID()} LaneIndex {laneIndex}{TrafficObjectFinderMessage.GetMessage()}");
                return;
            }
            ...
            foreach (Path path in trafficNode.ExternalLanes[laneIndex].paths)
            {
                if (path == null)
                {
                    Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad.name}', TrafficNode InstanceID {trafficNode.GetInstanceID()} External PathIndex {laneIndex} is null{TrafficObjectFinderMessage.GetMessage()}");
                    continue;
                }
                ...
                if (path.ConnectedTrafficNode == null)
                {
                    ...
                    Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad.name}', SourceNode '{sourceNodeName}' InstanceID {sourceId} Path '{path.name}' InstanceID {path.GetInstanceID()}. External PathIndex {laneIndex} connected node is null{TrafficObjectFinderMessage.GetMessage()}");
                    continue;
                }

                bool reversedConnection = path.ReversedConnectionSide;

                if (path.ConnectedTrafficNode && path.ConnectedTrafficNode.IsOneWay && path.ConnectedTrafficNode.IsEndOfOneWay)
                {
                ...
                {
                    int rightLanes = path.ConnectedTrafficNode.Lanes.Count;

                    if (rightLanes <= laneIndex && !path.ConnectedTrafficNode.IgnoreEmptyLanes)
                    {
                        Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad.name}', " +
                            $"Traffic node {trafficNode.name} InstanceID {trafficNode.GetInstanceID()} has missing connected lane index {path.ConnectedTrafficNode.name}. Connected Node Lane Count {rightLanes} Requested External LaneIndex {laneIndex}{TrafficObjectFinderMessage.GetMessage()}");
                        continue;
                    }
                }
                else
                {
                    int leftLanes = path.ConnectedTrafficNode.ExternalLanes?.Count ?? 0;

                    if (leftLanes <= laneIndex && !path.ConnectedTrafficNode.IgnoreEmptyLanes)
                    {
                        Debug.Log($"WARNING! TrafficLightCrossroad '{trafficNode.TrafficLightCrossroad.name}', " +
                            $"Traffic node {trafficNode.name} InstanceID {trafficNode.GetInstanceID()} missed connected lane index {path.ConnectedTrafficNode.name}. Connected Node Lane Count {leftLanes} Requested External LaneIndex {laneIndex}{TrafficObjectFinderMessage.GetMessage()}");
                        continue;
                    }
                }
