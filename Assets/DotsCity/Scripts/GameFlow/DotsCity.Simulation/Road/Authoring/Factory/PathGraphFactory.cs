...
namespace Spirit604.DotsCity.Simulation.Road.Authoring
{
    public static class PathGraphFactory
    {
        public static BlobAssetReference<PathBlobDataGraph> Create(
            ...
            Dictionary<int, List<int>> pathNeighborMap,
            Dictionary<int, List<int>> pathParallelMap)
        {
            var graphRef = CreatePathGraphInternal(pathConnectingMap, pathIntersectMap, pathNeighborMap, pathParallelMap);
            ...
            Dictionary<int, List<int>> pathParallelMap)
        {
            using (var builder = new BlobBuilder(Allocator.Temp))
            {
                ...
                for (int pathIndex = 0; pathIndex < pathConnectingMap.Count; pathIndex++)
                {
                    ...
                    if (pathNeighborMap.TryGetValue(pathIndex, out neighborIndexes))
                    {
                        var neighborPaths = builder.Allocate(ref nodeArray[pathIndex].NeighborPathsIndexes, neighborIndexes.Count);

                        for (int j = 0; j < neighborIndexes.Count; j++)
                        {
                            neighborPaths[j] = neighborIndexes[j];
                        }
                    }
