...
namespace Spirit604.DotsCity.Simulation.Road.Authoring
{
    ...
    public partial class TrafficPathConversionSystem : SimpleSystemBase
    {
        ...
        private Dictionary<int, List<int>> pathNeighborMap = new Dictionary<int, List<int>>();
        ...
        private void CompleteInitilization()
        {
            ...
            var graphRef = PathGraphFactory.Create(EntityManager, pathGraphEntity, trafficNodeConversionSystem.PathConnectingMap, pathIntersectMap, pathNeighborMap, pathParallelMap);
            ...
            for (int i = 0; i < sourceBuffer.Length; i++)
            {
                ...
                else
                {
                        ...
                        connectedPathInstanceId != -1)
                    {
                        ...
                        {
                            UnityEngine.Debug.Log($"Path Instance id '{sourceTempConnectionData.InstanceId}' Connected PathInstanceId '{connectedPathInstanceId}'. Path Point Connected path not found. Make sure that the path is connected to the correct lane index{TrafficObjectFinderMessage.GetMessage()}");
                            continue;
                        }
                ...
                for (int localPathIndex1 = minPathIndex; localPathIndex1 < maxPathIndex; localPathIndex1++)
                {
                    for (int localPathIndex2 = minPathIndex; localPathIndex2 < maxPathIndex; localPathIndex2++)
                    {
                        if (localPathIndex1 != localPathIndex2)
                        {
                            ...
                            if (!pathNeighborMap.ContainsKey(globalIndex1))
                            {
                                pathNeighborMap.Add(globalIndex1, new List<int>());
                            }

                            pathNeighborMap[globalIndex1].Add(globalIndex2);
                        }
                    }
                }
            }
