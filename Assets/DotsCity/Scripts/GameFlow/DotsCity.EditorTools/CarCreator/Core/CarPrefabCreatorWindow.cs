...
namespace Spirit604.DotsCity.EditorTools
{
    public class CarPrefabCreatorWindow : EditorWindowBase
    {
        ...
        private readonly string[] DefaultWheelFRWords = new string[] { "fr", "f.r", "f_r", "frontright", "front_right", "rightfront", "right_front" };
        private readonly string[] DefaultWheelFLWords = new string[] { "fl", "f.l", "f_l", "frontleft", "front_left", "leftfront", "left_front" };
        ...
        private readonly string[] DefaultWheelMWords = new string[] { "ml", "mr", "m.l", "m.r", "m_l", "m_r" };
                    ...
                    if (allWheelsFound)
                    {
                        ...
                        if (vehicleAuthoring != null)
                        {
                            ...
                            for (int j = 0; j < wheels.Count; j++)
                            {
                                ...
                                var dirToWheel = (wheel.transform.position.Flat() - carTemplatePrefab.transform.position.Flat()).normalized;
                                var forward = carTemplatePrefab.transform.forward;
                                var angle = Vector3.SignedAngle(forward, dirToWheel, Vector3.up);

                                var leftWheel = angle < 0;
                                ...
                                SetWheelMesh(prefabInfoData, wheel);
        ...
        private void FindWheels(GameObject sourcePrefab, out List<MeshRenderer> wheels, out List<MeshRenderer> extraWheels)
        {
            ...
            {
                var wheel = FindMeshByText(sourcePrefab.transform, wheelNameTemplates[type].WheelTemplates, type);
        ...
        private void SetWheelMesh(CarPrefabInfo carPrefabInfo, MeshRenderer wheelMesh)
        {
        ...
        private MeshRenderer FindMeshByText(Transform sourceParent, List<string> templates, WheelType wheelType = WheelType.None)
        {
                        ...
                        if (lowerTemplates[j].Length == 2)
                        {
                            ...
                            if (!add)
                            {
                                ...
                                if (meshName.Length > rightIndex &&
                                    Symbols.Contains(meshName[rightIndex]))
                                {
                                    add = true;
                                }
                            }
        ...
        private void TryToAddPrefabsToPool()
        {
            ...
            if (AddToExistPreset)
            {
                ...
                {
                    Debug.Log($"Preset {EntityType} not found");
                    return;
                }
            }
            else
