...
{
    [UpdateInGroup(typeof(PlayerPresentationGroup))]
    public partial class PlayerInteractCarSystem : BeginPresentationSystemBase
    {
            ...
            .ForEach((Entity playerStateEntity, in PlayerEnterCarStateComponent playerStateComponent) =>
            {
                if (playerStateComponent.EnterCarState == EnterCarState.Enter)
                {
                    ...
                    for (int i = 0; i < carGroup.CalculateEntityCount(); i++)
                    {
                        ...
                        InteractCarUtils.EnterCar(ref commandBuffer, ref entityManager, ref soundConfig, soundPrefabQuery.GetSingletonEntity(), newCarEntity, true, shouldIgnite, runtimeIndex, oldCarPosition);
                ...
                if (playerStateComponent.EnterCarState == EnterCarState.Leave)
                {
                    ...
                    InteractCarUtils.ExitCar(ref commandBuffer, ref soundConfig, soundPrefabQuery.GetSingletonEntity(), playerCarPosition, runtimeIndex);
