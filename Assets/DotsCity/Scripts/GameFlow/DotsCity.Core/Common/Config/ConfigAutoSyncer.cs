...
namespace Spirit604.DotsCity.Core
{
    public class ConfigAutoSyncer : IDisposable
    {
        ...
        public void Sync()
        {
            ...
            if (subScene == null || (!subScene.IsLoaded && !CityEditorSettings.AutoOpenClosedScene))
                return;
            ...
            if (subScene != null)
            {
                if (userDispose && !Application.isPlaying)
                {
                    ...
                    if (subSceneWasClosed && CityEditorSettings.AutoCloseScene)
                    {
                        ...
                        if (Selection.activeGameObject != null && Selection.activeGameObject.scene.isSubScene || Selection.activeGameObject == subScene.gameObject)
                            close = false;
